...
function DirectSolveFEM!( M, K, Q, Qt, M0, rhs, Vx, Vy, P, mesh, b)
    ...
    # file = matopen(string(@__DIR__,"/Milamin_Amatrix.mat"))
    # Amat = read(file, "A")
        ...
        # dropzeros!(K)
        # droptol!(K, 1e-8)
        # display(Plots.spy(Amat))
        # L = cholesky(Amat,check = false)
...
#      end
    ...
    if np==1 P   .= sol[(length(Vx)+length(Vy)+1):(length(Vx)+length(Vy)+mesh.nel)] end
    if np==3 
        P .= 0.0
        for i=1:np
            P   .+= 1//np .* sol[(length(Vx)+length(Vy) + (i-1)*mesh.nel + 1):(length(Vx)+length(Vy) + i*mesh.nel)] 
        end
    end
    return nothing
end
...
function main( n, nnel, θ, Δτ )
    ...
    mesh.nn     = size(GCOORD,2)
    mesh.xn     = GCOORD[1,:]
    mesh.yn     = GCOORD[2,:]
    mesh.phase  = convert(Vector{Int},Phases[:])
    mesh.bcn    = zeros(mesh.nn)
    bc = zeros(7062) 
    bc[Point_id[:].==1] .= 1
    mesh.bcn[1:7062] .= bc
    mesh.e2n    = ELEM2NODE'
    mesh.e2v    = ELEM2NODE[1:3,:]'
    mesh.nv     = maximum(mesh.e2v)
    mesh.xv     = GCOORD[1,1:mesh.nv]
    mesh.yv     = GCOORD[2,1:mesh.nv]
    mesh.nf_el  = 3
    D           = [1e-3;  1]
    ...
    mesh.ke[mesh.phase.==1] .= D[1]
    mesh.ke[mesh.phase.==2] .= D[2]
