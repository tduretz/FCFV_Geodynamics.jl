...
function ResidualOnFaces_v2_GPU!(F, Mesh_bc, Mesh_f2e, Mesh_dA_f, Mesh_n_x_f, Mesh_n_y_f, Mesh_vole_f, Mesh_vole, Mesh_e2f, Mesh_dA, Mesh_n_x, Mesh_n_y, Th, Te, qx, qy, ae, be, ze, tau, mesh_nf, mesh_nf_el)
    ...
    if idof <= mesh_nf
        ...
        qxel1  = (yes1==1) * -1.0/Mesh_vole[iel1]*ze[iel1,1]
        qyel1  = (yes1==1) * -1.0/Mesh_vole[iel1]*ze[iel1,2]
        for ifac=1:mesh_nf_el
            ...
            # Assemble
            Tel1  += (yes1==1) * (bcf!=1) *  dAi*taui*Th[Mesh_e2f[iel1, ifac]]/ae[iel1]
            qxel1 -= (yes1==1) * (bcf!=1) *  1.0/Mesh_vole[iel1]*dAi*ni_x*Th[Mesh_e2f[iel1, ifac]]
            qyel1 -= (yes1==1) * (bcf!=1) *  1.0/Mesh_vole[iel1]*dAi*ni_y*Th[Mesh_e2f[iel1, ifac]]
        end
        ...
        qxel2  = (yes2==1) * -1.0/Mesh_vole[iel2]*ze[iel2,1]
        qyel2  = (yes2==1) * -1.0/Mesh_vole[iel2]*ze[iel2,2]
        for ifac=1:mesh_nf_el
            # Face
            ...
            # Assemble
            Tel2  += (yes2==1) * (bcf!=1) *  dAi*taui*Th[Mesh_e2f[iel2, ifac]]/ae[iel2]
            qxel2 -= (yes2==1) * (bcf!=1) *  1.0/Mesh_vole[iel2]*dAi*ni_x*Th[Mesh_e2f[iel2, ifac]]
            qyel2 -= (yes2==1) * (bcf!=1) *  1.0/Mesh_vole[iel2]*dAi*ni_y*Th[Mesh_e2f[iel2, ifac]]
        end
