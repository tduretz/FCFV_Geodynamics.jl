function ComputeFCFV_o2(mesh, se, Tdir, tau, o2)
    ...
    for iel=1:mesh.nel  #@tturbo

        be[iel] = be[iel] + (o2==0)*mesh.Ω[iel]*se[iel] 
    ...
    if o2==1 # The remaining of this function is only done if second order FCFV is used
        ...
        @inbounds for e=1:mesh.nel 
            for i=1:mesh.nf_el
                ...
                for in=1:nen
                    dj[e,i,in] =  dAi * N[e,i,in] * Tdir[nodei]
                    rj[e,i,in] =  dAi * N[e,i,in]
                    for jn=1:nen
        ...
        @inbounds for e=1:mesh.nel
            ...
            for i=1:mesh.nf_el
                ...
                for in=1:nen
                    be_o2[e,in] += (bc==1) * taui * dj[e,i,in]
                end
...
function ComputeElementValues_o2(mesh, uh, ae, be, be_o2, ze, rj, mei, Tdir, tau, o2)
    ...
    @tturbo for iel=1:mesh.nel
        ...
        for ifac=1:mesh.nf_el
            ...
            qx[iel] -= (bc!=1) *  mesh.ke[iel]/mesh.Ω[iel]*dAi*ni_x*uh[mesh.e2f[iel, ifac]]
            qy[iel] -= (bc!=1) *  mesh.ke[iel]/mesh.Ω[iel]*dAi*ni_y*uh[mesh.e2f[iel, ifac]]
        end
        # inv(me) * ue
    ...
    @tturbo for iel=1:mesh.nel
        ...
        for ifac=1:mesh.nf_el 
            ...
            for jfac=1:mesh.nf_el
                ...
                Ke_ij =-dAi * (res * taui*tauj - mesh.ke[iel]/mesh.Ω[iel]*dAj*nitnj - taui*del);
                yes   = (bci!=1) & (bcj!=1)
            ...
            res2   = (pe[iel,ifac,1] * meid1 + pe[iel,ifac,2] * meid2 + pe[iel,ifac,3] * meid3)
            res    = (o2==0) * res1 + (o2==1) * res2
            fe_i   = (bci!=1) * -dAi * (mesh.ke[iel]/mesh.Ω[iel]*nitze - ti*Xi - res*taui)
